...
for i=1:length(p)
    ...
    OrAn1(i)=90-((p(i).Orientation) +0.8); %have to rotate the image by 0.8 degrees to align with x axis
    OrAn1(i)=(OrAn1(i)*(-1));
    if OrAn1(i) < -90
        OrAn1(i) = 180 + OrAn1(i);
    end
...
p2=regionprops(Iout,'orientation');

for i=1:length(p2)
    FUP_OrAn1(i)=90-(abs(p2(i).Orientation) +0.8);
